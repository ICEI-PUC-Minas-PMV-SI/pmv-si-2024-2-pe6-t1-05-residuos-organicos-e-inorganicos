...
import { View, TouchableOpacity, Image, Text, Linking, Alert } from 'react-native';
...
const Detail = () => {
  ...
  function handleComposeMail() {
    const subject = 'Interesse na coleta de resídous';
    const emailUrl = `mailto:${data.point.email}?subject=${encodeURIComponent(subject)}`;
    ...
    Linking.canOpenURL(emailUrl)
      .then((supported) => {
        if (supported) {
          ...
          Alert.alert('Erro', 'Nenhum aplicativo de email encontrado.');
        }
      })
      .catch((error) => console.error('Erro ao abrir email:', error));
